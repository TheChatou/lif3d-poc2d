/* ==========================================================================
   LIF2D — Composants contrôleurs physiques.
   Screw · RotaryPot · Slider · Encoder · PushButton · DrawWheel · Speaker · Toggle
   ========================================================================== */

const { useState, useRef, useEffect, useCallback } = React;

/* ---- Utilitaire drag pointer --------------------------------------------- */
// Capture le pointeur sur l'élément et renvoie les deltas pendant le glisser.
...
function RotaryPot({ label, value = 0.5, onChange, display, size = 74, accent = true }) {
  ...
  const onMove = (dx, dy) =>
    onChange && onChange(Math.max(0, Math.min(1, start.current + dy / 180)));
  const down = useDrag(onMove, () => { start.current = v; });

  const r = size / 2;

  // Graduations extérieures (11 ticks sur 270°)
  const ticks = [];
  for (let i = 0; i <= 10; i++) {
    ...
    <div className="lif-ctrl">
      <div className="lif-pot" style={{ width: size, height: size }} onPointerDown={down}>
        <svg width={size} height={size} className="lif-pot-dial">
          {ticks}
          {accent && (
            <path d={arc} fill="none" stroke="var(--brass)" strokeWidth="2.4" strokeLinecap="round" />
          )}
        </svg>
...
/* ---- Fader linéaire (vertical, 0–1) -------------------------------------- */
function Slider({ label, value = 0.5, onChange, display, height = 132 }) {
  const v     = Math.max(0, Math.min(1, value));
  const start = useRef(0);

  const onMove = (dx, dy) =>
    onChange && onChange(Math.max(0, Math.min(1, start.current + dy / (height - 28))));
  const down = useDrag(onMove, () => { start.current = v; });
...
/* ---- Encodeur cranté (rotation infinie, clic = reset) -------------------- */
function Encoder({ label, display, onStep, onClick, size = 66 }) {
  ...
  // 24 crans répartis sur 360°
  const detents = [];
  for (let i = 0; i < 24; i++)
    detents.push(<span key={i} style={{ transform: `rotate(${i * 15}deg)` }} />);
...
/* ---- Bouton poussoir avec LED centrale ----------------------------------- */
function PushButton({ label, active, color = 'var(--brass)', onClick, big = false }) {
  return (
    <div className="lif-ctrl">
      <button
        className={`lif-btn ${active ? 'is-on' : ''} ${big ? 'is-big' : ''}`}
        onClick={onClick}
        style={{ '--btn-led': color }}>
...
function DrawWheel({ axis = 'x', onStep, label }) {
  ...
  // Stries horizontales ou verticales selon l'axe
    ...
    <div className="lif-ctrl">
      <div className={`lif-wheel ${horizontal ? 'is-x' : 'is-y'}`} onPointerDown={down}>
        <div className="lif-wheel-surf"
          style={{
            backgroundImage: ridge,
            backgroundPosition: horizontal ? `${off}px 0` : `0 ${off}px`,
          }} />
